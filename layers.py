...
class GraphConvolution(Module):
    ...
    def reset_parameters2(self, num):
        ...
        numm = str(num)
        np.savetxt("Data/weight" + numm + ".txt", self.weight.detach().numpy())
...
#
#         self.W = nn.Parameter(torch.empty(size=(in_features, out_features)))  # (331,128)
#         nn.init.xavier_normal_(self.W.data, gain=1.414)  # 权重的正态分布
#         self.a = nn.Parameter(torch.empty(size=(2 * out_features, 1)))  # 注意力分数 (256,1)
#         nn.init.xavier_normal_(self.a.data, gain=1.414)  # 注意力分数正态分布
#
#     def forward(self, x, adj):
#         #adj = [[1,0,0],[1,1,0]]
#         #x = [[1,2,2,0,-1],[2,1,5,4,1],[2,1,0,2,3],[1,5,6,8,1],[2,3,5,6,1]]
#         # 公式中的部分    转移矩阵
#         #adj1 = []
#         #for i in range (adj.shape[0]):
#          #   for j in range(adj.shape[1]):
#           #      if adj[i][j] == 1:
#           #          adj1[i][j]=exp(np.dot(np.hstack((x[i],x[adj.shape[0]+j])),self.att))
#            #     else:
#             #        adj1[i][j] = 0
#         h = self.linear(x)  # 先经过线性变换
#         N = h.size(0)
#
#         # 计算注意力分数
#         a_input = torch.cat([h.repeat(1, N).view(N * N, -1), h.repeat(N, 1)], dim=1).view(N, -1, 2 * self.out_features)
#         e = F.leaky_relu(torch.matmul(a_input, self.a).squeeze(2))  # 每个输入的注意力权重
#         zero_vec = -9e15 * torch.ones_like(e)
#         attention = torch.where(torch.Tensor(x) > 0, e, zero_vec)
#         attention = F.softmax(attention, dim=1)
#         A = F.dropout(attention, self.dropout, training=self.training)
#         degree1 = torch.sum(adj, dim=1)
#         degree2 = torch.sum(adj, dim=0)
#         degree = torch.cat((degree1, degree2), dim=0)
#         D = torch.diag(degree)
#         # print(D)
#         IN_D_inv_sqrt = torch.sqrt(torch.inverse(torch.eye(x.size(0)) + D))
#         # print(IN_D_inv_sqrt)
#         # 等效度矩阵
#         Dd1 = torch.sum(A, dim=1)
#         Dd = torch.diag(Dd1)
#         #Dd = torch.diag_embed(Dd)
#
#         #print(Dd)
#
#         Tgcn = torch.matmul(torch.matmul((torch.matmul(IN_D_inv_sqrt, Dd)), torch.Tensor(A)).T, IN_D_inv_sqrt)
#         #print(Tgcn)
#         x = torch.mm(Tgcn, x)
#         x = torch.mm(x, self.W)
#         x = F.relu(x)# 执行矩阵乘法
...
#         # print(Tgcn)
#         # h_prime = torch.matmul(attention, h)
#         # F.elu(h_prime)#前向传播是用输入的特征矩阵乘以权重再乘以注意力分数，，，返回训练得到的向量
#
#         return x
...
#
# class CRFAttentionLayer(nn.Module):      #  CRF attention层
#     def __init__(self, in_dim, alpha,beta, K=2):
#         super(CRFAttentionLayer, self).__init__()
#         self.alpha = alpha
#         self.beta = beta
#         self.K = K
#         self.att_proj = nn.Linear(in_dim, in_dim)
#
#     def forward(self, Q, sim_mat,alpha=None, beta=None):
#         """
#         Q: 原始节点表示 (N, d)
#         sim_mat: 相似性矩阵 (N, N)，如疾病/微生物的结构相似性
#         """
